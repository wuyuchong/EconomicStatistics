...
```{r}
dat = read.csv("data copy.csv", header = TRUE)
dat = dat[2:ncol(dat)]
...
```{r fig.align="center", fig.cap="离职与在职两类员工绩效（红色代表离职）", out.width="80%"}
ggplot(dat_complete, aes(x = PerformanceScore, fill = Terminate)) +
  geom_bar(stat = "count", alpha = 0.6, position = "dodge") +
  theme_minimal() +
  scale_fill_manual(values = c("#037418", "darkred")) +
  labs(y = "percentage")
```
...
我们将**主动辞职**和**被迫离职**的标记为离职，与在职相对应，生成一个虚拟变量。
...
```{r}
confusion = confusionMatrix(data = test$Terminate,
                reference = testPred,
                positive = "1")
...
```{r}
summary(dat_complete$Terminate)
207 / (207 + 103)
```

可以看到：准确率大致为： `r round(confusion$overall[1], 3)`，置信区间 XXX ，并不算高。
